				...
				(groupBy == 'none' ? [null] : this.getTaxonomy(groupBy) || [null]).forEach(taxonomyTerm => {
					...
					if(orderBy == 'type') {
						this.displayPeople(format, peopleInGroup.sort((personA, personB) => {
								...
								const
									personAJobTypeName = PeopleListBlockElement.getTaxonomyName(jobTypeTaxonomy, personAJobTypeData[0]['meta']['drupal_internal__target_id']),
									personBJobTypeName = PeopleListBlockElement.getTaxonomyName(jobTypeTaxonomy, personBJobTypeData[0]['meta']['drupal_internal__target_id']);
